...
import com.android.launcher3.helper.InterruptibleInOutAnimator;
 ...
 * 一个子 布局(放置 子控件 + 分组部件)
 */
public class CellLayout extends BaseViewGroup
{
    ...
    public CellLayout(Context context, AttributeSet attrs, int defStyle)
    {
        ...
        setWillNotDraw(false); // 通知,onDraw将会被执行
        setClipToPadding(false); // 通知,如果有padding,不对子控件进行裁剪
        ...
        mCellWidth = mCellHeight = -1; // -1
        mFixedCellHeight = -1; // -1
        mWidthGap = mOriginalWidthGap = 0; // 0
        mHeightGap = mOriginalHeightGap = 0; // 0
        mMaxGap = Integer.MAX_VALUE; // 2147483647
        mCountX = (int)grid.numColumns; // 8
        mCountY = (int)grid.numRows; // 5
        mOccupied = new boolean[mCountX][mCountY];
        mTmpOccupied = new boolean[mCountX][mCountY];
        mPreviousReorderDirection[0] = INVALID_DIRECTION; // -100,-100
        ...
        mHotseatScale = (float)grid.hotseatIconSize / grid.iconSize; // 0.8
        ...
        mReorderHintAnimationMagnitude = (REORDER_HINT_MAGNITUDE * grid.iconSizePx); // 9.599999
        
        mNormalBackground.setFilterBitmap(true);
        mActiveGlowBackground.setFilterBitmap(true);
        
        // Initialize the data structures used for the drag visualization.
        final TimeInterpolator mEaseOutInterpolator = new DecelerateInterpolator(2.5f); // Quint ease out 减速插值器
        ...
        final int duration = res.getInteger(R.integer.config_dragOutlineFadeTime); // T1,900
        final float fromAlphaValue = 0;
        final float toAlphaValue = (float)res.getInteger(R.integer.config_dragOutlineMaxAlpha); // T1,128.0
        
        // 数组,初始化掉
        Arrays.fill(mDragOutlineAlphas, fromAlphaValue);
        
        // mDragOutlineAnims 初始化
        for (int i = 0; i < mDragOutlineAnims.length; i++) // length = 4
        {
                ...
                public void onAnimationUpdate(ValueAnimator animation)
                {
                    ...
                    if (outline == null)
                    {
                        ...
                        if (debug)
                        {
                            ...
                            LogFileUtil.d(TAG,
                                "anim " + thisIndex + " update: " + val + ", isStopped " + anim.isStopped());
                        }
    ...
    private void regionToCenterPoint(int cellX, int cellY, int spanX, int spanY, int[] result)
    {
        ...
        result[1] =
            vStartPadding + cellY * (mCellHeight + mHeightGap) + (spanY * mCellHeight + (spanY - 1) * mHeightGap) / 2;
    }
    ...
    */
    ...
    public static void getMetrics(Rect metrics, int paddedMeasureWidth, int paddedMeasureHeight, int countX, int countY)
    {
        ...
        metrics.set(grid.calculateCellWidth(paddedMeasureWidth, countX),
            grid.calculateCellHeight(paddedMeasureHeight, countY),
            0,
            0);
    }
    ...
    {
        LogFileUtil.v(Launcher.TAG, "CellLayout onMeasure");
        LauncherAppState app = LauncherAppState.getInstance();
        DeviceProfile grid = app.getDynamicGrid().getDeviceProfile();
    ...
    */
        ...
        View dragView, ItemConfiguration currentState)
    {
        ...
        findNearestArea(boundingRect.left,
            boundingRect.top,
            boundingRect.width(),
            boundingRect.height(),
            direction,
    ...
    private void copySolutionToTempState(ItemConfiguration solution, View dragView)
    {
        ...
        markCellsForView(solution.dragViewX,
            solution.dragViewY,
            solution.dragViewSpanX,
            solution.dragViewSpanY,
            mTmpOccupied,
            true);
    }
    ...
    private void beginOrAdjustHintAnimations(ItemConfiguration solution, View dragView, int delay)
    {
        ...
        for (int i = 0; i < childCount; i++)
        {
            ...
            if (c != null)
            {
                ReorderHintAnimation rha =
                    new ReorderHintAnimation(child, lp.cellX, lp.cellY, c.x, c.y, c.spanX, c.spanY);
                rha.animate();
            }
        }
    }
    ...
    */
        ...
        View dragView, int[] resultDirection)
    {
        ...
        getViewsIntersectingRegion(targetDestination[0],
            targetDestination[1],
            spanX,
            spanY,
            dragView,
        ...
        regionToRect(dropRegionRect.left,
            dropRegionRect.top,
        ...
        int[] result, int resultSpan[], int mode)
    {
        ...
        ItemConfiguration noShuffleSolution =
            findConfigurationNoShuffle(pixelX,
                pixelY,
                minSpanX,
                minSpanY,
                spanX,
     ...
     */
    private boolean findCellForSpanThatIntersectsIgnoring(int[] cellXY, int spanX, int spanY, int intersectX,
    ...
    {
        LogFileUtil.v(Launcher.TAG, "rectToCell width = " + width + ",height = " + height + ",result = " + result);
        LauncherAppState app = LauncherAppState.getInstance();
        DeviceProfile grid = app.getDynamicGrid().getDeviceProfile();
