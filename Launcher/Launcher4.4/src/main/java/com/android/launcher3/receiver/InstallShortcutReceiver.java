...
public class InstallShortcutReceiver extends BaseReceiver
{
            ...
            try
            {
                JSONStringer json =
                    new JSONStringer().object()
                        .key(DATA_INTENT_KEY)
                        .value(info.data.toUri(0))
                        .key(LAUNCH_INTENT_KEY)
                        .value(info.launchIntent.toUri(0))
                        .key(NAME_KEY)
                        .value(info.name);
                if (info.icon != null)
                {
                    ...
                    json =
                        json.key(ICON_KEY).value(Base64.encodeToString(iconByteArray,
                            0,
        ...
        if (!installQueue.isEmpty())
        {
            ...
            if (result == INSTALL_SHORTCUT_IS_DUPLICATE)
            {
                Toast.makeText(context,
                    context.getString(R.string.shortcut_duplicate, duplicateName),
