...
public class DynamicGrid
{
	...
	{
		LogFileUtil.v(Launcher.TAG, "DynamicGrid construct start");
		...
		mProfile =
				new DeviceProfile(context, deviceProfiles, mMinWidth, mMinHeight, widthPx, heightPx, awPx, ahPx, resources);

		LogFileUtil.v(Launcher.TAG, "DynamicGrid construct end");
	}
	...
	public String toString()
	{
		return "-------- DYNAMIC GRID ------- \n" + "Wd: " + mProfile.minWidthDps + ", Hd: " + mProfile.minHeightDps
				...
				+ mProfile.cellWidthPx + ", ch: " + mProfile.cellHeightPx + ", hc: " + mProfile.numHotseatIcons + ", his: "
	...
	public static class DeviceProfile
	{
		...
		{
			LogFileUtil.v(Launcher.TAG, "DynamicGrid -> DeviceProfile contruct start");
			...
			LogFileUtil.v(Launcher.TAG, "DynamicGrid -> DeviceProfile contruct end");
		}

		public void updateFromConfiguration(Resources resources, int wPx, int hPx, int awPx, int ahPx)
		{
			LogFileUtil.v(Launcher.TAG, "updateFromConfiguration -> wPx = " + wPx + ",hPx = " + hPx + ",awPx = " + awPx
					+ ",ahPx = " + ahPx);
			isLandscape = (resources.getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE); // false
			isTablet = resources.getBoolean(R.bool.is_tablet); // true
			...
			{
				allAppsNumRows =
						(availableHeightPx - pageIndicatorOffset - 4 * edgeMarginPx)
								/ (iconSizePx + iconTextSizePx + 2 * edgeMarginPx);
			}
			else
			{
				allAppsNumRows = (int) numRows + 1; // 6
			}
			allAppsNumCols =
					(availableWidthPx - padding.left - padding.right - 2 * edgeMarginPx) / (iconSizePx + 2 * edgeMarginPx); // 7
		}
		...
		public void layout(Launcher launcher)
		{
			...
			else if (isTablet())
			{
				...
				int hotseatGap =
						(int) Math.max(0, (gridWidth - (numHotseatIcons * hotseatCellWidthPx)) / (numHotseatIcons - 1));
				lp.gravity = Gravity.BOTTOM;
				lp.width = LayoutParams.MATCH_PARENT;
				lp.height = hotseatBarHeightPx;
				hotseat.setPadding(2 * edgeMarginPx + gridGap + hotseatGap,
						0,
