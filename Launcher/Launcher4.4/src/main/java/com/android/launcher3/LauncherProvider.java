...
public class LauncherProvider extends ContentProvider
{
	...
	private static class DatabaseHelper extends SQLiteOpenHelper
	{
					...
					if (intentUri != null)
					{
						try
						{
							...
							if (uri != null)
							{
								...
								if ((Intent.ACTION_VIEW.equals(intent.getAction()) || actionQuickContact.equals(intent.getAction()))
										&& (data.startsWith("content://contacts/people/") || data.startsWith("content://com.android.contacts/"
										+ "contacts/lookup/")))
								{
		...
		private void normalizeIcons(SQLiteDatabase db)
		{
			...
			try
			{
				...
				while (c.moveToNext())
				{
					...
					{
						Bitmap bitmap =
								Utilities.resampleIconBitmap(BitmapFactory.decodeByteArray(data, 0, data.length), mContext);
						if (bitmap != null)
						{
		...
		{
			Cursor c =
					db.rawQuery("SELECT MAX(" + LauncherSettings.WorkspaceScreens._ID + ") FROM " + TABLE_WORKSPACE_SCREENS,
							null);
			...
			try
			{
						...
						&& type != XmlPullParser.END_DOCUMENT)
				{
					...
					if (TAG_INCLUDE.equals(name))
					{
						...
						if (LOGD)
						{
							String content = String.format("Skipping <include workspace=0x%08x>", resId);
							Log.v(TAG, content);
						}
						continue;
					}
					...
					if (LOGD)
					{
						final String title = a.getString(R.styleable.Favorite_title2);
					...
					{
						String title;
						int titleResId = a.getResourceId(R.styleable.Favorite_title2, -1);
						if (titleResId != -1)
						{
		...
		private long addUriShortcut(SQLiteDatabase db, ContentValues values, TypedArray a)
		{
			...
			final int titleResId = a.getResourceId(R.styleable.Favorite_title2, 0);
