...
class Supply2(private val mBitmap: Bitmap, random: Random, private var mMapRect: Rect) : ISupply {
    /**
     * 0	未开始状态
     * 1	正常状态
     * 2	透明状态
    ...
    override fun start(durateTime: Float, frizTime: Float): Boolean {
        if (0 == mState) {
            if (mTime > frizTime) {
                ...
                positionX = mLeft.toFloat()
                positionY = mMapRect.top - mBitmap.height.toFloat()
                return true
            } else {
                mTime += durateTime
            }
    ...
    companion object {
        const val SupplyRes = R.drawable.supply2
    }
