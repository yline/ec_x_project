...
public class Supply1 implements ISupply{
	public final static int SupplyRes = R.drawable.supply1;
	/**
	 * 0	未开始状态
	 * 1	正常状态
	 * 2	透明状态
	...
	public Supply1(Bitmap bitmap, Random random, Rect rect) {
		this.mBitmap = bitmap;
		this.mMapRect = rect;
	...
	public boolean start(float durateTime, float frizTime) {
		if (0 == mState) {
			if (mTime > frizTime) {
				this.mTime = 0;
				this.mState = 1;
				this.mRect = new Rect(mLeft, mMapRect.top - mBitmap.getHeight(),
						mLeft + mBitmap.getWidth(), mMapRect.top);
