...
public class Bullet2 implements IBullet{
	private final static int ATK = 2;
	public final static int bulletRes = R.drawable.bullet2;
	...
	public Bullet2(Bitmap bitmap, int bgTop, int bottom) {
		this.mBitmap = bitmap;
		this.mTop = bgTop;
		this.mBottom = bottom;
	...
	public boolean fireBullet(int x, int y, float durateTime, float frizTime) {
		if (BulletState.unStart == mState) {
			...
			if (mCounter.judge) {
				...
				this.mRect = new Rect(x - mBitmap.getWidth()/2, y - mBitmap.getHeight()/2,
						x + mBitmap.getWidth()/2, y + mBitmap.getHeight()/2);
